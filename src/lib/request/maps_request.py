...
def address_to_coord(address: str, cityinfo: bool=False) -> tuple[str, float, float]:
    ...
    """
    url = "https://api.geoapify.com/v1/geocode/search?text=" + address + "&apiKey=" + token.geoToken()
    ...
    """
        ...
        Returns:
            statuscode, distance, duration (str, int, float): 狀態碼及計算過後的交通距離和花費時間(單位：公里,秒)
                statuscode = {"T"|"F"}: T: 正常, F: 異常
    """
    url = "https://maps.googleapis.com/maps/api/distancematrix/json?origins=" + str(coord1[0][1][1]) + "%2C" + str(coord1[0][1][0]) + "&destinations=" + str(coord2[0][1][1]) + "%2C" + str(coord2[0][1][0]) + "&mode=" + mode +"&key=" + token.googleMapsToken()
    payload = {}
    headers = {}
